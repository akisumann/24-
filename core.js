...
function makeStats(base){return Object.fromEntries(STATS.map(s=>[s,Math.max(1,Math.round(base*(.5+srandom()))) ]))}
...
let favoredMotherId=null; // 大寵愛：この大淫義で「胸＋尻−腰×0.5」が最大の母のid（retirement.jsが毎大淫義に設定）。
function makeChild(m){
  ...
  const child={id,family:m.family,given:pick(GIVEN),age:0,maxStats,inheritance,body:{height:Math.round(m.body.height*.7+162*.3+rand(-10,10)),bust:Math.round(m.body.bust*.8+bodyBase()*.2+rand(0,10)),waist:Math.round(m.body.waist*.5+62*.5+rand(-7,7)),hip:Math.round(m.body.hip*.8+bodyBase()*.2+rand(0,10)),hair:srandom()<.7?m.body.hair:pick(HAIR)},origin:'神の娘・国家育成対象',mother:full(m),motherId:m.id,generation:(m.generation||1)+1,skills:makeSkills(maxStats,id)};
