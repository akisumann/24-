...
function body(){return{height:rand(150,166),bust:rand(74,94),waist:rand(54,74),hip:rand(81,101),hair:pick(HAIR)}}
...
function makeChild(m){
  ...
  const child={id,family:m.family,given:pick(GIVEN),age:0,maxStats,inheritance,body:{height:Math.round(m.body.height*.7+162*.3+rand(-6,6)),bust:Math.round(m.body.bust*.8+bodyBase()*.2+rand(-10,10)),waist:Math.round(m.body.waist*.8+62*.2+rand(-10,10)),hip:Math.round(m.body.hip*.8+bodyBase()*.2+rand(-10,10)),hair:Math.random()<.7?m.body.hair:pick(HAIR)},origin:'神の娘・国家育成対象',mother:full(m),skills:makeSkills(maxStats,id)};
