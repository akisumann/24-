...
let favoredMotherId=null; // 最寵：この大儀で胸＋尻が最大の母のid（retirement.jsが毎大儀に設定）。
function makeChild(m){
  const usedGodLevel=godLevel,id=nextId++,maxStats={},inheritance={};
  // 最寵された母（最も豊満＝神が最も激しく求めた者）の娘は、能力乱数の下限だけ 0.5→0.6 に上がる（上限1.5は据え置き）。
  // 最寵の有無で srandom の消費回数は変わらない（各項目1回）ため、シード再現性は保たれる。
  ...
  if(favored)child.favored=true; // 最寵の子（娘バッジ用）
