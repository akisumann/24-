...
function body(){const b=Math.round(bodyBase());return{height:rand(150,177),bust:b,waist:62,hip:b,hair:pick(HAIR)}}
...
function makeChild(m){
  ...
  const child={id,family:m.family,given:pick(GIVEN),age:0,maxStats,inheritance,body:{height:Math.round(m.body.height*.7+162*.3+rand(-6,6)),bust:Math.round(bodyBase()),waist:62,hip:Math.round(bodyBase()),hair:Math.random()<.7?m.body.hair:pick(HAIR)},origin:'神の娘・国家育成対象',mother:full(m),skills:makeSkills(maxStats,id)};
