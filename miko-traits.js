...
(function(){
  ...
  // 口調（喋り方）：一人称・語尾・話し方の個体差。id基準の決定論（乱数非消費）。
  // 性的要素を含まない人となりの軸なので、育成対象を含む全年齢に表示してよい。
  const SPEECH=[
    ['おっとり丁寧','一人称「わたくし」。「〜ですわ」「〜ますの」と柔らかく上品に話す。'],
    ['快活で砕けた','一人称「あたし」。「〜だよ！」「〜だね！」と明るく元気に話す。'],
    ['凛と硬質','一人称「私」。「〜だ」「〜である」と簡潔で武人めいた物言い。'],
    ['古風な巫女口調','一人称「わらわ」。「〜じゃ」「〜のう」と古めかしく話す。'],
    ['甘えた幼い調子','一人称は自分の名。「〜なの」「〜だもん」と舌足らずに甘える。'],
    ['物静かで寡黙','一人称「私」。言葉少なで「…ええ」「…そう」と静かに応じる。'],
    ['姉御肌の伝法','一人称「あたい」。「〜だぜ」「〜さ」と勝ち気にさばけて話す。'],
    ['のんびり天然','一人称「わたし」。「〜かなぁ」「〜だねぇ」と間延びしてマイペース。'],
    ['小生意気','一人称「うち」。「〜でしょ？」「〜じゃない」と生意気に言い返す。'],
    ['律儀で堅物','一人称「自分」。「〜であります」「〜します」と生真面目に話す。'],
    ['大和撫子な淑やか','一人称「わたし」。「〜でございます」「〜くださいまし」と控えめで奥ゆかしい。'],
    ['ぶっきらぼう','一人称「私」。「〜。」「別に」と言葉を切り詰め、素っ気なく話す。'],
    ['お嬢様高飛車','一人称「わたくし」。「〜ですのよ」「〜なさい」と勝ち気で高慢に話す。'],
    ['明るい訛り','一人称「うち」。「〜やで」「〜やん」「〜ちゃう？」と陽気な訛りで話す。'],
    ['おどおど気弱','一人称「わたし」。「あ、あの…」「〜、かも…」と小声で自信なげに話す。'],
    ['理屈っぽい','一人称「私」。「つまり〜」「〜と考えられる」と理詰めで説明的に話す。'],
    ['芝居がかった大仰','一人称「我」。「〜なのだ！」「ふっ…」と大げさに芝居っぽく話す。'],
    ['人懐こく甘ったるい','一人称「あたし」。「ねぇねぇ〜」「〜だよぉ」と距離が近く甘えて話す。'],
    ['きっぷのいい早口','一人称「あたし」。「〜っしょ」「〜じゃん」とテンポよくまくし立てる。'],
    ['謎めいて思わせぶり','一人称「私」。「…さあ、どうかしら」「見えているわ」と掴みどころなく話す。']
  ];
  function speechTone(p){return SPEECH[hp(p.id,8,SPEECH.length)];}
  ...
  // 18歳で種縁談、20歳までに全員結婚（成人は例外なく夫あり）。夫との子：20歳0〜1／〜27で+1〜3／〜34で+1〜3。
  function hp(id,salt,mod){return (id*31+salt*17)%mod;}
  function isMarried(p){return p.age>=20;}                          // 20歳までに全員結婚＝成人は皆夫あり
  function kidsByHusband(p){
    let n=0;
    if(p.age>=20)n+=hp(p.id,2,2);                                  // 20歳までに0〜1（嫁いで間もない）
  ...
  function traitsHtml(p){
    ...
    h+='<div class="trait-grp"><div class="trait-grp-h">人となり</div>';
    h+=`<div>口調：<b>${esc(speechTone(p)[0])}</b>　<span class="muted">${esc(speechTone(p)[1])}</span></div>`;
    h+='</div>';
  ...
  try{if(typeof window!=='undefined')window.MikoTraits={traitsHtml,volOf,clitMm,clitSens,clitStage,speechTone};}catch(e){}
